...
| agent\_sandbox\_enabled | The status of the Agent Sandbox addon, which isolates untrusted code execution (requires GKE version 1.35.2-gke.1269000 or later and a gVisor-enabled node pool for Standard clusters) | `bool` | `false` | no |
...
| pod\_snapshot\_config | Whether the Pod Snapshot addon is enabled for this cluster. | `bool` | `false` | no |
